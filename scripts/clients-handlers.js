...
module.exports.editClient = (request, response) => {
    ...
    connection.query(
        query,
        [request.body.name, request.body.address, request.body.postCode, request.body.email, request.body.nif, request.body.id],
        function (err, rows) {
            if (err) {
                console.log(err);
...
module.exports.createClient = (request, response) => {
    ...
    connection.query(
        query,
        [request.body.name, request.body.address, request.body.postCode, request.body.email, request.body.nif],
        function (err, rows) {
            if (err) {
                console.log(err);
                response.sendStatus(500);
